<template>
    <div class="content">
            ...
            <el-table-column label="操作">
                <template #default="scope">
                    <el-tooltip content="查看样本" placement="bottom" effect="light">
                        <img src="@/assets/images/ckybxx.png" @click="infos=openInfo(scope,'detail');showDetail()" class="table-btn"/>
                    </el-tooltip>
                </template>
            </el-table-column>
        </el-table>
        ...
        >
            <template #default>
                <span> 共 {{ collectList.length }} 条记录&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;当前显示第 {{ pageSize*currentPage>collectList.length?collectList.length:pageSize*currentPage }} 条记录 &nbsp;&nbsp;</span>
            </template>
        </el-pagination>

        <!--采集按钮对话框-->
        <el-dialog v-model="col_dialog"  
            width="500"
...
const showDetail = () => {
    ...
    content.value = sample["样本类型"],
    hospital.value = sample["采集医院"]
...
const pageData = reactive({
    datalist: computed(()=>{
        return collectList.value.slice(
...
const openInfo = (item,type) => {
    if(type=='dialog'){
        if(item.row["采集状态"]=='未知'&&item.row["运输状态"]=='未知'&&item.row["接收状态"]=='未知')
            col_dialog.value = true;
        else if(item.row["采集状态"]=='已完成'&&item.row["运输状态"]=='未知')
            tran_dialog.value = true;
        else if(item.row["采集状态"]=='已完成'&&item.row["运输状态"]=='运输中'&&item.row["接收状态"]=='未接收')
            acc_dialog.value = true;
    }
    return item.row
}
