            ...
            <div class="fridgeBOX" style="width: 631px">
                <div class="fridgeBoxContent">
                    <div class="content" style="height: 400px">
                        <p style="text-align: center" v-for="item in props.msg" :key="item.level">
                            <span
                                v-for="subItem in item.children"
                                :key="subItem.level"
                                @click="getClickItem(item, subItem)"
                                :style="{ 'background-color': choose === item.label + subItem.label ? '#f8e3c5' : '#fff' }"
                                >{{ subItem.label }}<br />{{ container.getCellsNumRate(subItem.children, 'line') }}</span
                            >
                        </p>
                    </div>
                    <div class="door">
                        <div class="doorContent" id="doors_content">
                            <p v-for="item in props.msg" :key="item.level">
                                <span :style="{ background: container.getColor(item.children, 'frame') }">{{ item.label }}<br />{{ container.getCellsNumRate(item.children, 'frame') }}</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="infor_1">
                    ...
                    <div class="infor_text">
                        <div style="width: 70%; font-size: 14px">
                            ...
                            <p style="padding: 0 2px">样本空间占有率:{{ container.getCellsNumRate(props.msg, 'frame') }}</p>
                ...
                <div class="infor_1" style="display: block; margin-top: 20px" v-if="container.displayItem">
                    ...
                    <div class="infor_text">
                        <div style="width: 70%; font-size: 14px">
                            ...
                            <p style="padding: 0 2px">样本空间占有率:{{ container.getCellsNumRate(directionItem.children, 'line') }}</p>
                        </div>
                    </div>
                </div>
            </div>
